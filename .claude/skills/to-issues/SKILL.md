---
name: to-issues
description: Break a parent PRD into a flat, ordered list of native GitHub sub-issues, each a tracer-bullet vertical slice the Ralph loop picks up one at a time in its own session. Quizzes the user on granularity and order, then creates the sub-issues with the afk label and attaches them to the PRD. Project-local variant of /to-issues for this repo's PRD-as-parent + native sub-issues multi-session workflow. Use when the user wants to break a parent PRD (given by issue number or URL) into implementation sub-issues. Argument is the parent PRD issue number.
---

# To Issues (project)

Break a parent PRD into a flat list of native GitHub sub-issues, in execution order. Each sub-issue is a tracer-bullet vertical slice that the Ralph loop will pick up one at a time.
...
1. **Create the issue:**
   ...
   ```
   gh issue create --title "<title>" --label "afk" --body "$(cat <<'EOF'
   <body — see template>
   EOF
   )"
   ```

   Every sub-issue gets the `afk` label. This prints the new issue URL. Capture the issue number.
...
3. **Attach as sub-issue of the PRD:**
   ...
   ```
   Use `-F` (not `-f`): the sub-issues endpoint requires `sub_issue_id` to be a JSON **integer**, and `-f` always sends string values, which fails with HTTP 422 (`is not of type integer`). `-F` sends the numeric literal untyped.

Sub-issues get the `afk` label (applied at creation, above). 
